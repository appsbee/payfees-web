 ...
 * Time: 12:25 PM
 */
class Studentmanagement extends Base_SchoolAdmin_Controller {
	...
	public function uploadstudent() {
		...
		if (isset($id)) {
			...
			$this->_loadView('school/student_management/upload_student');
	...
	public function studentdetails() {
		...
		//echo '<pre>';print_r($this->data["schools"]);die();
		$this->data["userName"] = $this->session->user_name;
		$this->_loadView('school/student_management/student_details');
	}
		...
		if (isset($_POST["submit"])) {
			if ($_FILES['studentdetails']['type'] == 'application/vnd.ms-excel') {
				...
				for ($i = 2; $i <= count($arr_data) + 1; $i++) {
					...
					$studentdata['school_name'] = isset($arr_data[$i]['A']) ? $arr_data[$i]['A'] : '';
	...
	public function advancesearch() {
		...
		if ($this->input->post('school_id') != "") {
			...
			$this->_loadView('school/student_management/advancesearch');

		} else {
			redirect("school/Studentmanagement/uploadstudent", 'refresh');
		}
	...
	public function excelupload() {
		...
		if ($this->input->post('uploadfile') == "2") {
			...
			//echo '<pre>';
			//print_r($schoolname['school_name']);
			//die();
			$data = array();
			//echo '<pre>';
			...
			if (isset($_POST["submit"])) {
				if ($_FILES['fileupload']['type'] == 'application/vnd.ms-excel') {
					...
					for ($i = 2; $i <= count($arr_data) + 1; $i++) {
						...
						$result = $this->Studentdetails->studentdetailsimport($studentdata);
						if ($result == '0') {
							$this->session->set_flashdata('error_details', 'Upload Proper Excel File');
							redirect("school/Studentmanagement/uploadstudent/", 'refresh');
						}
						//echo $result;
					}
