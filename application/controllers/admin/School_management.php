...
class School_management extends Base_Admin_Controller {
	...
	public function index() {
		...
		if ($this->input->post("submit")) {
			...
			$uploadeschholfile = $this->input->post("uploadeschholfile", true);
			if (isset($uploadeschholfile) && $uploadeschholfile == "schoolLogo") {
				//$this->uploadSchoolFiles('school_logo');
			} else if (isset($uploadeschholfile) && $uploadeschholfile == "schoolImage") {
				if (isset($_FILES['school_img']) && $_FILES['school_img']['name'] != "") {
					//$this->uploadSchoolFiles('school_img');
				}
			} else {
				$this->view_all();
			}
	...
	public function uploadSchoolFiles($field_name) {
		if (!is_dir('uploads/')) {
	...
	public function getschoolname() {
		...
		$school_name = $this->school->getschoolname($keywords);
		header('Content-Type: application/json; charset=utf-8');

		echo json_encode($school_name);
		exit;
		//echo '<pre>';
		//print_r($school_name);
