...
class Schoolapi extends CI_Model {
	...
	public function schoolDetailsByID($schoolId, $guardianregister_id) {
		...
		if ($query->num_rows() > 0) {
			...
			$messages_query = $this->db->select('school_message.school_message_id,school_message.title,school_message.message,school_message.message_date,guardianregister.guardianregister_id')->from('school_message,school_message_notification')->join('guardianregister', 'guardianregister.guardianregister_id= school_message_notification.guardian_id', 'left')->where('school_message.school_id', $schoolId)->where('guardianregister.guardianregister_id', $guardianregister_id)->group_by("school_message.school_message_id")->order_by('school_message.message_date', 'DESC')->limit('5')->get();
